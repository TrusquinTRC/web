...
$db->exec("CREATE TABLE IF NOT EXISTS movie_ads(id INTEGER PRIMARY KEY NOT NULL, type TEXT, pages TEXT, images TEXT, text TEXT)");
$res = $db->query("SELECT * FROM movie_ads");
$rows = $db->query("SELECT COUNT(*) as count FROM movie_ads");
$row1 = $rows->fetchArray();
$HOSTa = $lurl = (isset($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] !== "off" ? "https" : "http") . "://" . $_SERVER["HTTP_HOST"] . dirname($_SERVER["PHP_SELF"]) . "/ads/default_ad.png";
$numRows = $row1["count"];
if ($numRows == 0) {
    $db->exec("INSERT INTO movie_ads(type, pages, images, text) VALUES('image','dashboard','https://t.me/bet3inc','https://www.dropbox.com/s/nrt1wpozl8a54sd/logo.png?raw=1','Bet3 Creations')\r\t");
}

if (isset($_GET["delete"])) {
    $db->exec("DELETE FROM movie_ads WHERE id=" . $_GET["delete"]);
    $db->close();
    header("Location: movieads.php");
}
include "includes/header.php";
echo "\r<!-- [ Main Content ] start -->\r    <div class=\"pcoded-main-container\">\r        <div class=\"pcoded-wrapper\">\r            <div class=\"pcoded-content\">\r                <div class=\"pcoded-inner-content\">\r                    <div class=\"main-body\">\r                        <div class=\"page-wrapper\">\r                            <!-- [ Main Content ] start -->\r                            <div class=\"row\">\r                    <!-- Column -->\r                    <div class=\"col-lg-12\">\r                        <div class=\"card mb-4\">\r\t\t\t\t\t\t\t<div class=\"card-header bg-primary py-3\">\r                \t\t\t\t<h4 align=\"center\" class=\"m-0 font-weight-bold text-white\"><i class=\"fas fa-images\" style=\"font-size:20px\">&nbsp;&nbsp;</i>Movie AD Manager</h4>\r                \t\t\t</div>\r                         <div class=\"card-body\">\r\t\t\t\t\t\t\t <br>\r\t\t\t\t\t\t\t\t<div align=\"center\" class=\"col-12\">\r\t        \t\t\t\t\t\t<a  href=\"./add_movie_ads.php\"><button class=\"btn btn-danger btn-icon-split\" name=\"submit\" type=\"submit\"><span class=\"icon text-white-50\"><i class=\"fa fa-plus\"></i></span>&nbsp;&nbsp;<span class=\"text\"> Upload AD Image</span></button></a>\r    \t\t\t\t\t\t\t</div>\r\t\t\t\t\t\t\t <br>\r\t\t\t\t\t\t<div class=\"table-responsive\">\r\t\t\t\t\t\t\t<table class=\"table table-striped table-sm\">\r\t\t\t\t\t\t\t<thead align=\"center\">\r\t\t\t\t\t\t\t\t<tr>\r\t\t\t\t\t\t\t\t<th><strong>Ad Image</strong></th>\r\t\t\t\t\t\t\t\t<th></th>\r\t\t\t\t\t\t\t\t<th><strong>Edit</strong></th>\r\t\t\t\t\t\t\t\t<th><strong>Delete</strong></th>\r\t\t\t\t\t\t\t\t</tr>\r\t\t\t\t\t\t\t</thead>\r\t\t\t\t\t\t\t";
while ($row = $res->fetchArray()) {
	echo "\r\t\t\t\t\t\t\t<tbody align=\"center\">\r\t\t\t\t\t\t\t\t<tr class=\"text-primary\">\r\t\t\t\t\t\t\t\t\r\t\t\t\t\t\t\t\t<td><img src=\"";
    echo $row["images"];
    echo "\" alt=\"\" border=3 height=140 width=600></img></td>\r\t\t\t\t\t\t\t\t<td>";
    echo "</td>\r\t\t\t\t\t\t\t\t<td><a href=\"./edit_movie_ads.php?update=";
    echo $row["id"];
    echo "\"><i class=\"fas fa-edit\" style=\"font-size:20px;color:white\"></i></a></td>\r\t\t\t\t\t\t\t\t<td><a href=\"./movieads.php?delete=";
    echo $row["id"];
