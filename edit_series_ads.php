...
$res = $db->query("SELECT * \r\t\t\t\t  FROM series_ads \r\t\t\t\t  WHERE id='" . $_GET["update"] . "'");
$row = $res->fetchArray();
if (isset($_POST["submit"])) {
    $db->exec("UPDATE series_ads SET  \t\t\r\t\t\t\t\t\t\t\timages='" . $_POST["images"] . "'\r\t\t\t\t\t\t  WHERE \r\t\t\t\t\t\t\t  \tid='" . $_POST["id"] . "'");
    $db->close();
...
echo $row["id"];
echo "\">\r                      
           </div>\r                                <div class=\"form-group\">\r                          <label class=\"bmd-label-floating\"><strong>Image Url</strong></label>\r                          <input type=\"images\" class=\"form-control text-primary\" name=\"images\" value=\"";
echo $row["images"];
